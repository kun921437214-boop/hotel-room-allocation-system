  ...
  identities.forEach((identity) => {
    ...
    dates.forEach((date, dateIndex) => {
      roomTypes.forEach((roomType, typeIndex) => {
        ...
        const count = needs.filter((need) => needMatchesIdentity(need, identity) && needStaysOnDate(need, date) && need.roomType === roomType).length;
...
function addIdentitySheet(workbook, identity, needs, dates, roomTypes, capacities, sequenceByNeed) {
  ...
  dates.forEach((date, dateIndex) => {
    ...
    roomTypes.forEach((roomType, typeIndex) => {
      const col = startCol + typeIndex;
      const available = totalCapacity(capacities, date, roomType);
      const used = needs.filter((need) => needStaysOnDate(need, date) && need.roomType === roomType).length;
  ...
  dates.forEach((date, dateIndex) => {
    roomTypes.forEach((roomType, typeIndex) => {
      const col = firstMatrixCol + dateIndex * roomTypes.length + typeIndex;
      const count = needs.filter((need) => needStaysOnDate(need, date) && need.roomType === roomType).length;
...
async function buildOverviewWorkbook(payload = {}) {
  ...
  identities.forEach((identity) => {
    const identityNeeds = needs.filter((need) => needMatchesIdentity(need, identity));
