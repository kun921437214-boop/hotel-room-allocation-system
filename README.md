...
- `入住需求查看`：一行一条入住需求，方便看人员、日期、状态和已分配房间。
- `酒店房间查看`：一行一间房，方便看酒店、房号、房型、可住人数和可用时间。
- `分房记录查看`：一行一条分房记录，方便核对谁住哪间房。
- `变更记录查看`：一行一条变更，方便追踪换房、取消、延期等事项。

这些查看表是从 `系统同步数据` 自动整理出来的镜像，适合查看、筛选和排序；网站实际读写仍以 `系统同步数据` 的 JSON 内容为准。
